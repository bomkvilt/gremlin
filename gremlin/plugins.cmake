...
function(GNP_on pluginManager event)
...
macro(GNP_onGlobal pluginManager event)
    GNP_get(callbacks ${pluginManager} "events.${event}")
    foreach(callback  ${callbacks})
        GNP_callEventGlobal(${callback} ${event} ${ARGN})
        endforeach()
    endmacro()
...
function(GNP_callEvent callback)
...
macro(GNP_callEventGlobal callback)
    # message("call: ${callback}::event(${ARGN})")
    if (EXISTS ${callback})
        include(${callback})
        event(${ARGN})
